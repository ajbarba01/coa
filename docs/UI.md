...
- **Take the interaction library for mechanics, not for management.** The kit adopts it where hand-rolling
  ...
  decision, not the omission it looks like.
...
- **A theme is a scale swap at full quality.** A new theme replaces the whole surface scale and status set to the
  same bar as sand-dark — never a partial recolor. Motion is a token axis, not a per-component decision.
  **There is no density axis, by decision.** The user-visible density control was _removed_, not deferred: all it
  ever did was scale a type ramp that no longer exists, so it had stopped changing anything. Written down so its
  absence is read as the decision it was rather than as an accidental regression to restore.
