...
_Design date: 2026-07-18. Status: approved, pre-implementation (mockup-gated)._
...
- **Add from defaults** — pick catalog models not yet in your list.
...
- **RPC verbs** — `listModels` / `addModel` / `editModel` / `removeModel` / `setModelHidden` /
  `listDefaultCatalog`, over the store, mirroring the auth-verb pattern in `buildAuthHandlers`.
- **Renderer** — replace the temporary client-only `hiddenModels` state with store-backed reads; the chat
  chip and agent picker keep reading `state.data.models`, now the effective list.
...
0. **High-fidelity in-app mockup pass** (maintainer-requested gate) — covers the per-provider model editor,
   the chat chip, and the agent-config picker, to confirm no surface is missed **before** implementation.
