...
| ~~Depth >1 subagent nesting~~ **SHIPPED — no longer deferred** (CHAT-2, M8 D150) — orchestrating and rendering subagents that spawn their own subagents | Superseded: D122's depth-1 bound was dropped in favor of the daemon-global cost cap as the sole fan-out bound (`docs/adr/0032`); the CHAT-2 render model's "depth-general" design is exercised, not just designed — proven against a grandchild fixture. | — (row kept only so a reader of this table's history does not mistake the strikethrough for a deletion; discovery (`find_agent`) has since shipped too — the remaining gap is the cost roll-up's RPC producer, parked, see ROADMAP.md item M). |
