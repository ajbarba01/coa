...
| **C2** | **Subagent-stream linkage** — `parentTurn` field + `subagent` lifecycle frames (spawn/running/idle/done/rollup) | M8 Push + R-7 keying | CHAT-2, R-7.a | No new orchestration — renders the existing depth-1 fan-out (D122/D96). The per-worktree store already keys child streams; only the linkage field is new. |
...
| **Depth >1 subagent nesting** (CHAT-2/CHAT-11) — rendering (and orchestrating) subagents that spawn their own subagents | D122 bounds the fan-out to depth-1 (one attributable, cost-bounded layer); the CHAT-2 render model is depth-general but v1 shows one level. | multi-level orchestration ships (gated by the v2 autonomy stack — D91/D92), at which point the nesting tree extends. |
...
18. **Subagent-nesting legibility at depth (the CHAT-2 residual).** Nested rendering can confuse (the documented Roo
   complaint: invisible hierarchy + cost roll-up). **Mitigation:** v1 is depth-1 (D122); the parent frame carries a
   per-subagent tools/tokens/$/status roll-up; a path-to-root is shown. **Residual:** even one level of nesting plus a
   tool-heavy child can overwhelm a small pane — surfaced honestly via the focus/denoise control (CHAT-5), not hidden.
