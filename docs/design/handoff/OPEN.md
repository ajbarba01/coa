...
| **C2** | ~~Subagent-stream linkage — `parentTurn` field + `subagent` lifecycle frames (spawn/running/idle/done/rollup)~~ **SHIPPED, by a different mechanism.** Session lineage (`SessionMeta.parent`/`.root`, M8 D150) + console-side grouping, not the turn-level `parentTurn`/`subagent` `TurnFrame` link this row originally proposed (still schematized, still unwired). A read-time transcript join (`foldTreeToTranscript`) is built and unit-tested, and now has its first production caller: a completed child's own final answer is folded through it into the parent's completion notice (`docs/adr/0038`, capped/sanitized) — the parent's own manual read of `events.ndjson` remains the fallback for anything past the cap. Depth is unbounded (D122 superseded — `docs/adr/0032`), not the depth-1 this row assumed. | M8 D150 (`packages/core/src/session/`) | `docs/adr/0032`/`0034`/`0038`, CHAT-2 | The tree cost roll-up was investigated for an RPC producer and **parked, not shipped** — the ledger has no session-identifying field to join against at all (not merely an unwired join); `listSessions` does not surface `costUsd`; see ROADMAP.md item M for the full finding. |
...
| ~~Depth >1 subagent nesting~~ **SHIPPED — no longer deferred** (CHAT-2, M8 D150) — orchestrating and rendering subagents that spawn their own subagents | Superseded: D122's depth-1 bound was dropped in favor of no in-system fan-out bound at all (`docs/adr/0032`, superseded in turn by `docs/adr/0035`); the CHAT-2 render model's "depth-general" design is exercised, not just designed — proven against a grandchild fixture. | — (row kept only so a reader of this table's history does not mistake the strikethrough for a deletion; discovery (`find_agent`) and agent-to-agent messaging (`send_message`/`list_agents`, `docs/adr/0039`, M8 D151) have since shipped too — the remaining gap is the cost roll-up's RPC producer, parked, see ROADMAP.md item M). |
