...
`AsyncIterable<string>`, so a steer reaches the in-flight query. Either way M8 branches only on the abstract
string, never on the backend (ADR 0002/0004). A connection is a **stateless, reattachable
...
SESSION LIFECYCLE
  ...
  steerSession(id, text) -> { steered: boolean }        ✅ CHAT-10 (both backends; see docs/adr/0012) — the pure-API path queues the steer for the `loop-driver` to drain at its next safe boundary; the Claude SDK path (held-open strategy) pushes it as another streamed user turn into the live query's derived input feed, where the SDK QUEUES it to run at the current turn's boundary (no mid-turn inject — live-verified in `streaming-smoke.live.test.ts`; true mid-turn redirect is the barge-in follow-up, see docs/adr/0012). An empty steer is a no-op. Console steer affordance still to wire
