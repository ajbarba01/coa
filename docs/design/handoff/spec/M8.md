...
`createSession` open across turns, feeding it the session's successive user turns (and any steer) as a derived
`AsyncIterable<string>`. A steer carries a **mode** (`queue | barge-in`, `docs/adr/0012`): `queue` runs after the
current turn; `barge-in` stops the current turn now and runs the steer next. On the held-open path M8 realizes
barge-in via the backend's turn-level interrupt handle (reported up neutrally, never an SDK type) plus a framed
push, and tracks a per-turn boundary count so the injected steer turn resolves the correct awaited turn. Either
way M8 branches only on the abstract strategy verdict, never on the backend (ADR 0002/0004). A connection is a **stateless, reattachable
...
SESSION LIFECYCLE
  ...
  steerSession(id, text, mode?) -> { steered: boolean } ✅ CHAT-10 + barge-in (both backends; see docs/adr/0012). `mode` (`queue` | `barge-in`, default `queue`) is realized per strategy, never by branching on the backend. Held-open (Claude SDK): `queue` pushes another streamed user turn the SDK runs at the current turn's boundary; `barge-in` calls the SDK's turn-level `query.interrupt()` (stops the current turn, keeps the query ALIVE) then pushes a framed steer (`[The user interrupted to steer you] …`) that runs next — a per-turn boundary count keeps the injected steer turn resolving the correct awaited turn, and the interrupt's own `error_during_execution` result is suppressed (SC-1, live-verified in `barge-in-smoke.live.test.ts`). Pure-API: `barge-in` = the existing `drainSteer` inject at the loop's next round-trip boundary; `queue` = a new `drainQueuedSteer` inject at the point the close-gate would end the turn (run after the current turn). An empty steer is a no-op. Console steer affordance still to wire
