...
- **Completion reaches the parent as a system notice, not a message.** `renderChildEnded` produces a `Delivery`
  ...
  (`completed`/`errored`/`stopped`). For a `completed` outcome the notice also carries a bounded, sanitized
  excerpt of the child's own final answer (`docs/adr/0038`, extending `docs/adr/0033`): a read-time fold of the
  child's event log (`foldTreeToTranscript` + `latestAssistantText`), capped and flattened the same way an
  `errored` outcome's `detail` already is, never text the child asserts about itself through any tool-reachable
  seam. The full transcript remains directly readable for anything past the cap.
- **Discovery.** `find_agent` (on-demand, granted via the Core package alongside `spawn_agent`) searches the same
  live roster `spawn_agent` resolves against — by `ref`/`name`/`description`, query optional — so a model can
  learn what it may spawn before attempting one, rather than only discovering the roster reactively off an
  unknown-ref retry.

**D151 — agent-to-agent messaging: a mesh, no fan-out bound (locked; `docs/adr/0039`).** Any two sessions sharing a
family-tree root may message each other via `send_message` (kernel, non-blocking — returns immediately, never a
reply) — hierarchy is provenance only, never a permission boundary. `list_agents` (on-demand) is the live roster,
distinct from `find_agent`'s registry read: every tree member relationship-labeled (`self`/`parent`/`child`/
`ancestor`/`descendant`/`other`) and liveness-graded (`'observed'` when currently registered or an end this
process itself watched; `'advisory'` when neither — never a fabricated observation). Messages ride a durable
append-only log, one file per root (`.coa/local/messages/`), deliberately not the change-event spine (the same
category argument `docs/adr/0033` gives for a completion notice). Delivery realizes on the two mechanisms the
session layer already has — `Delivery`/`DeliveryQueue` (`docs/adr/0030`) for a receiver mid-turn, a fresh queued
turn ("wake") for every other receiver state (idle / not-registered / not-yet-started collapse to one mechanism,
since none can be reached by a queue push alone). **Deviates from D150's "cost-cap-bounded" framing as written**:
`docs/adr/0035` had already archived the cap's deny path three days prior, so messaging inherits the SAME
no-fan-out-bound acceptance `spawn_agent` already lives under, not a second cap that no longer exists. Three
`TurnFrame` kinds (`subagent-spawn`/`subagent-completion`/`subagent-message`) announce spawn/completion/message
events on the daemon's live push stream for the console — additive to the wire schema, live-only (never
persisted; a reload will not show one), a scoped floor named explicitly in `docs/adr/0039` rather than solved
here.
A child shares its root's worktree by default; isolation is opt-in via `spawn_agent`'s `isolate` flag, which binds
a real `git worktree add`-backed copy instead (`docs/adr/0037`) — accepted as the shared-root default when
isolation isn't requested, since v1 is attended and every write from either agent still passes `PreToolUse`. The
tree's cost roll-up
(`SessionMeta.root`/`LedgerRecord.root` sum) is implemented and unit-tested, but **shipping its RPC producer was
investigated and parked**, not merely undone: the ledger's allow-list carries no session-identifying field at
all, and even `root` is attached only to a child's own record (every sibling under one root shares the same
value, and a root session's own direct spend is untagged entirely) — see ROADMAP.md item M for the full finding
and what a real fix needs. `listSessions` does not surface `costUsd` today.
...
TIMELINE (human-only; M1 owns, exposed here — D98)
  listTimeline() -> Checkpoint[]                        (b) — exists on M1; the console Timeline panel's read
  (pin/unpin/rewind are archived to archive/undo-plumbing/ — revival path: turn-level undo)
...
LIBRARY (skills + MCP servers — declarative stores over on-disk discovery; local only, no registry. Every mutation verb refuses — an ordinary error, surfaced like any other — when the target scope's store file did not load cleanly: a lossy load is never saved back over the content it dropped, so a broken `library.json` is fixed or removed by hand, never silently rewritten)
  listLibrary() / rescanLibrary() -> LibraryView        ✅ — ONE read: store entries (live-resolved references, materialized copies + on-demand hash drift) ∪ discovered-but-unlinked sources (~/.claude/skills, .claude/skills, ~/.codex/skills; the three Claude MCP config layers, precedence local > project > user, shadowing surfaced) ∪ diagnostics. Reads are always fresh (no cache, no watcher), so both verbs are the same scan; rescan is the explicit refresh gesture
  linkLibrary(kind, scope, source, name?) -> LibrarySummary     ✅ — link-as-reference (the default): a pointer to the source path (+ serverName for mcp); reads always reflect the source. Refuses a missing source or a duplicate (kind, name)
  copyLibrary(kind, source, name?) -> LibrarySummary    ✅ — copy-into-project: materializes into `<root>/.coa/library` (a skill as a real committable SKILL.md; an mcp server embedded in its record) with provenance {sourcePath, contentHash}; calling it again on the same entry IS the one-click re-sync (re-copy, hash refreshed, enabled choice kept)
  unlinkLibrary(kind, scope, name) -> { removed }        ✅ — removes the record (+ a skill copy's materialized directory); a second unlink is a no-op, not a failure
  setLibraryEnabled(kind, scope, name, enabled) -> LibrarySummary   ✅ — surfacing-only: a disabled entry stays listed and is excluded from injection/session wiring (strict-superset — off is a pass-through, never a cage)
  listSkills() -> { skills: InvocableSkill[] }          ✅ — the composer's `/skill` picker: every effective skill (enabled + resolvable, project shadowing personal by case-folded name) as {name, description, scope}. Injection rides `createSession`: optional `skills:[{name, delivery}]` (per-send override of the agent definition's list) and `invokeSkills:[name]` (an explicit one-turn load — the body reaches THAT turn's context even in disclosure mode, persisted as its own `system` frame above the user turn; an unknown name refuses the send). The resolved selection joins the drift key (`PromptConfig.skills`) and the stored compilation; library MCP entries (enabled, scope-folded) are delivered per turn to the backend — native `mcpServers` on the Claude SDK (the in-process `coa` server wins a name collision), a typed error-frame degrade on pure-API backends
...
`M1.graph.*`/`listTimeline`, `M7.capState`/`ledger`/`decisionLog`). Reads are safe-to-call-repeatedly and
