...
first send for a given id starts the daemon-owned turn loop, which drains the queue one turn at a time, running
each through the SAME per-turn `createSession` (D121) unchanged in substance — the neutral facade both backends
run, so nothing here branches on backend (ADR 0002/0004). A connection is a **stateless, reattachable
...
SESSION LIFECYCLE
  ...
  steerSession(id, text) -> { steered: boolean }        ✅ (pure-API only) / ⚠ CHAT-10 (SDK path, follow-up) — queues a mid-turn turn the pure-API `loop-driver` drains at its next safe boundary; no `inject`-now mode yet (queue-only). SDK-path steering, and the console steer affordance, are deferred to a future plan bundled with interactive multi-turn (both need streaming-input mode) — the Claude adapter has no `drainSteer` seam
