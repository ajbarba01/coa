...
- `runLoop(sessionConfig)` — drive one rented agent turn-loop for a session. The session `input` is neutral: a
  ...
  session's turns, reading each turn (and each steer M8 pushes into the same feed) off the iterable; and delivers
  ...
  the query. The one-shot `string` path is unchanged (D85). **Turn-level interrupt (barge-in, `docs/adr/0012`):**
  ...
  distinct from the whole-query `AbortController` used for `interruptSession`). M8 drives a `barge-in` steer
  through it; the one-shot string path never reports one. Live-verified: interrupting a running turn yields the
