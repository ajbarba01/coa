...
- `runLoop(sessionConfig)` — drive one rented agent turn-loop for a session. The session `input` is neutral: a
  ...
  session's turns off the iterable; a steer no longer rides this feed (it rides the delivery port below,
  `docs/adr/0031`) — and delivers
  prior memory as a first-turn preamble when switching into Claude (`withHistoryPreambleStreaming`). **One enriched
  output stream (`docs/adr/0010`, executed):** the adapter emits ONE stream via `onTurn(frame, full?)` — the lossy
  UI frame plus, for a `tool_result`, the `full` body (persistence-only, never on the wire — D57). M8 appends it to
  the single append-only event log; the provider transcript is a read-time fold of that log. The prior second
  writer (`onBackendMessages` + the accumulated transcript + `dropTrailingDanglingToolCall`) is retired — a
  mid-turn crash is handled by the fold's read-time repair, not a per-`result` flush. Cost is still settled at EACH
  SDK `result`. **Termination:** the loop ends when the coa-owned
  input iterable closes — after the last turn's `result` — so M8 closing the feed (at session close) terminates
  the query. The one-shot `string` path is unchanged (D85). **Turn-level interrupt (user-Stop, `docs/adr/0012`):**
  under streaming input the adapter reports a neutral turn-interrupt handle UP via `onTurnInterrupt` (a
  `() => Promise<void>`, the SDK's `Query.interrupt` — stops the current turn while keeping the query alive,
  distinct from the whole-query `AbortController` used for `interruptSession`). This handle exists solely for
  the user-Stop path (`setInterruptClosure`) now that barge-in is removed (`docs/adr/0031`) — nothing else
  calls it; the one-shot string path never reports one. Live-verified: interrupting a running turn yields the
  turn's partial blocks (A1) then an `error_during_execution` result, and the query stays open for the next turn.
  **Known gap:** `createClaudeAdapter` (the real composition root) still does not wire this port, so a live
  Claude user-Stop degrades to a whole-query abort instead — named in `docs/adr/0031`.
- `registerTools(catalogue)` — register M6's governed tool catalogue (the MCP surface) into the loop.
- **External MCP delivery (the library seam):** the session init carries the library-resolved external server
  map (`mcpServers: Record<name, McpServerEntry>`, M0-typed). The Claude adapter maps each entry onto the SDK's
  native `mcpServers` option beside the in-process `coa` server (`coa` wins a name collision — governance is
  never shadowed by config) and keeps `strictMcpConfig` on, so this option stays the ONE channel MCP config
  reaches the loop through. A pure-API backend has no MCP runtime and surfaces the fact as a typed error frame
  naming the unavailable servers at loop start (strict-superset: surfaced degrade, never a silent drop). coa
  manages config + surfacing only; the SDK owns the actual MCP runtime (P8).
...
- `deliverReminder(r: Reminder, at: 'session-start'|'prompt'|'post-tool')` — deliver the reminder M3 decides
  (D108/D133). The SDK's types admit a `role: 'system'` message, but the CLI transmits it without obeying it, so
  it is not a usable mid-session channel: `session-start` folds into the `systemPrompt`; `prompt` → `UserPromptSubmit`
  hook `additionalContext`; `post-tool` → `PostToolUse` hook `additionalContext` keyed
...
- `renderNative(neutralConfig) -> backendConfig` — render M5's backend-neutral config to SDK-native options
  (`systemPrompt`, `disallowedTools`, `tools`, `canUseTool`).
...
adapter: it wraps the Claude Agent SDK and gets the rented loop, tools, hooks, and sandbox for free. It enables
`includePartialMessages`, mapping each partial-message content-block delta to a delivery-only `text-delta`/
`thinking-delta` frame (`docs/adr/0013`); the settled assistant message still yields the canonical `text`/
`thinking` frames that persist.
...
actually drives their turn-loop. Both stream via SSE (`stream: true`): `complete()` yields text/reasoning deltas
as they arrive and returns the assembled settled result (`docs/adr/0013`), so the driver renders live deltas
while only the settled frame persists. The app-side `createAdapter` (in `apps/cli`, not `core` — backend packages are
