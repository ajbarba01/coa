...
The tool catalogue (the MCP surface M9 registers):
  ...
  RETRIEVE (read; structure-first, with whole-file fallback)
    ...
    find_references(symbol)      -> reference sites                       [reads M1 graph]
      (get_symbol / outline / find_references are implemented but currently unregistered —
       the symbol index they read has no producers yet, so they could only return empty
       results; they rejoin the registered catalogue when the symbol layer is fed)
