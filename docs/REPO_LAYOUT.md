...
| M9 Runtime Adapter           | `packages/spi` + `packages/loop-driver` + `packages/adapter-claude-sdk` + `packages/adapter-openai-compat` | Ports (types) + the shared pure-API loop driver + the SDK backend + the one thin OpenAI-compatible backend (DeepSeek and LongCat as `ProviderSpec` objects). |
