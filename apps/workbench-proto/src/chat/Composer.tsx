...
export interface ComposerProps {
  ...
  disabled?: boolean;
  /** Showcase seam: pre-fill the field so a state can be pinned for review. */
  defaultText?: string;
        ...
        <div className="flex items-center gap-1 border-t border-s4 px-2 py-1.5">
          ...
          {!running ? (
            ...
            <>
              {hasText && (
                <>
                  <Button onClick={queue} title="waits for the turn to end (⏎)">
                    queue
                  </Button>
                  <Button
                    variant="outline"
