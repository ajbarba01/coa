...
interface UiState {
  ...
  rawMode: boolean;
  /** Local record of the approvals the operator resolved, keyed by requestId — overlays
   *  a `resolved` receipt onto a transcript-derived approval frame once answered
   *  (advisory: surfacing only — the daemon owns the real decision). A LIVE ask never
   *  reads this map; it clears by leaving the session slice's pending queue. */
  ...
  /** Console-local notes per session (a model switch, an attachment, an invoked skill),
   *  positioned by the count of turn-derived frames already appended when recorded. */
...
export const useConsoleUi = create<UiState>(() => ({ ...EMPTY }));
