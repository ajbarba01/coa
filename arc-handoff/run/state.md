...
| `arc/stage3` | c921f28 | Stage 3's rolling integration branch, off `arc/architecture` @ 5c232df. Q11 (root/home seam, incl. the verifier-found web-tools.ts gap), Q14 (queue leak), and all 4 UX-polish clusters merged and gated green (2951 tests, depcruise 419/1217, docs-check 60). Each feature/fix lands on its own `arc/f<N>-<name>` (or `arc/qNN-<name>`) branch, merged into this one as it gates green; PR opens against `arc/architecture` once the stage is substantially done |
