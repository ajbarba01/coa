...
One KNOWN DEBT section — honest and specific, but RE-DERIVED FROM THE TREE, not copied from any brief: several long-standing items were fixed today (the session-layer restructuring and its connection-ownership bug, the suite's load-flakes, a duplicated grammar table, leaked git probe stderr). Read the recent git log and verify each candidate still exists before listing it. Likely-surviving candidates to CHECK: hand-rolled interrupt/steer flag machine; console store re-render blast radius, transcript triplication, tab-switch reload race, the 2s poll. CAUTION — the following were on this list and a charter has since targeted every one of them: daemon-crash wedging renderer run-state, the daemon manager discarding error reasons, composition pass-through layering, the silent fire-and-forget mutation catches, and the reconciler error-latch. Read the recent git log and CHECK EACH against the tree; list only what genuinely survives, and say so plainly if a fix landed but is only partial.
KNOWN DEBT items that are current as of 2026-08-08 and should be carried (verify, then state in plain language): (1) the daemon composition root takes a root override, but several sites inside the same startup path still bake in the process working directory and the home directory, so a daemon served against another root reads its agent registry and writes its conversation store into the process cwd — the checkout accumulates a .coa directory as a result; (2) the auth handlers compute key-file paths from the home directory directly at nine sites instead of from their own injected dependencies, so that seam is only half-honoured; (3) three desktop panel suites have been seen failing intermittently under parallel load and are not yet root-caused — note this WITHOUT weakening the green-every-run rule, since a failure of exactly that shape was chased on 2026-08-08 and turned out to be a real defect, not a flake. Each item one or two sentences, no codenames.
