...
| M10 Console | Partial / rich | Electron shell, the `console-ui` kit, live chat wired to a real governed session, rich tool cards, live drift/cache-staleness banners, a Stop button + Esc that cooperatively interrupts the running turn (`interruptSession`); an auto-expanding, smooth-collapsing (and now correctly-timed: collapses when output begins) reasoning block in the muted trace color, and a cascaded blur+rise entrance for non-streamed blocks (tool cards/results/plans), behind a single `reveal` config seam. | Live approvals/deny (blocked on M8's R-12), Longform + graph (React Flow) views, the system-prompt viewer; console steer affordance (deferred, see "Coa-agent hardening"); **the live per-word streaming reveal for agent output text (a block-split `StreamingMarkdown` — the per-word-via-react-markdown attempt was invisible + laggy and was reverted; see `.superpowers/sdd/streaming-per-word-reveal-handoff.md`)**. | 
