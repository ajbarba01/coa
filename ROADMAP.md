...
| M4 Context Engine | Partial | SSOT drift detection and origin-anchor verification are live. The grounding producer (`ground()`) only ever queried a permanently-empty symbol index — no production path populated it — and is parked in `archive/grounding-producer/`. | L-ASM (assembly/sizing) is spike-gated on the v0 calibration turn (keystone B below); G5 model-confirm needs M9's `runEval`. |
