...
| M4 Context Engine | Partial | SSOT drift detection and origin-anchor verification are live. The grounding producer (`ground()`) only ever queried a permanently-empty symbol index — no production path populated it — and is parked in `archive/grounding-producer/`. The spec-conformance pair (`publishGenerationSeam` and `governanceFor`) was never wired at either end — nothing wrote the authority edge and nothing read it — and is parked in `archive/spec-conformance/`; the code-health producer (`health()`) was never constructed or registered and is parked in `archive/code-health/`, leaving its composition helper `composeProfile` caller-less in the tree. | L-ASM (assembly/sizing) is spike-gated on the v0 calibration turn (keystone B below); G5 model-confirm needs M9's `runEval`. |
