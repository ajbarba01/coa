...
- **Advisory-first: the close gate is the only block.** The one thing in the whole system that can refuse is
  the close gate — the check that can decline to let the agent call the work finished. Everything else is
  advisory or surfacing, and no other component may deny. Per-tool deny rules are demotable pipeline policy
  running through the same single channel, not a second standing block; a governance check that throws refuses
  the call rather than admitting an unchecked one.
  - **Spend is accounted, never capped.** Every settled result is charged and recorded in the ledger with the
    ...
    ever set — machinery that described a stop the product could not perform. **Subagent fan-out is therefore
    unbounded** — no depth limit, no width limit, no spend bound. The operator's stop and the provider plan's
    own usage limit are the real backstops, which is acceptable while every run is attended. **A bound on
    fan-out is a roadmap item, not a shipped feature** — do not describe one as if it exists.
