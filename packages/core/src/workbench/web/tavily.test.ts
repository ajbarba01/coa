...
describe('makeTavilyFetch', () => {
  ...
  it('POSTs to /extract with a Bearer key and markdown format', async () => {
    ...
    const fetchImpl = (async (u: string, i: typeof init) => {
      ...
      return {
        ...
        status: 200,
        headers: { get: () => null },
        json: async () => ({ results: [{ raw_content: 'md' }] }),
        text: async () => '',
      };
    }) as unknown as typeof fetch;
  ...
  it('maps 429 to rate-limit and 432/433 to quota', async () => {
    expect(
      await makeTavilyFetch({ apiKey: 'k', fetchImpl: fakeFetch({ status: 429 }) }).fetch(
        'https://x.test',
      ),
    ).toEqual({ status: 'limit', kind: 'rate-limit' });
    expect(
      await makeTavilyFetch({ apiKey: 'k', fetchImpl: fakeFetch({ status: 432 }) }).fetch(
        'https://x.test',
      ),
    ).toEqual({ status: 'limit', kind: 'quota' });
    expect(
      await makeTavilyFetch({ apiKey: 'k', fetchImpl: fakeFetch({ status: 433 }) }).fetch(
        'https://x.test',
      ),
    ).toEqual({ status: 'limit', kind: 'quota' });
  });
  ...
  it('maps 401 and other non-ok statuses to an error', async () => {
    expect(
      await makeTavilyFetch({ apiKey: 'k', fetchImpl: fakeFetch({ status: 401 }) }).fetch(
        'https://x.test',
      ),
    ).toMatchObject({ status: 'error' });
    expect(
      await makeTavilyFetch({ apiKey: 'k', fetchImpl: fakeFetch({ status: 500 }) }).fetch(
        'https://x.test',
      ),
    ).toMatchObject({ status: 'error' });
  });
  ...
  it('maps an empty/failed extract to an error', async () => {
    expect(
      await makeTavilyFetch({
        apiKey: 'k',
        fetchImpl: fakeFetch({ status: 200, json: { results: [] } }),
      }).fetch('https://x.test'),
    ).toMatchObject({ status: 'error' });
  });
  ...
  it('SC-1: a fetch throw becomes an error outcome', async () => {
    ...
    expect(await makeTavilyFetch({ apiKey: 'k', fetchImpl }).fetch('https://x.test')).toMatchObject(
      { status: 'error' },
    );
  });
});
...
describe('makeTavilySearch', () => {
  it('maps results[] to neutral SearchHits (content → snippet)', async () => {
    const provider = makeTavilySearch({
      ...
      fetchImpl: fakeFetch({
        status: 200,
        json: { results: [{ title: 'T', url: 'https://x.test', content: 'snip' }] },
      }),
    });
  ...
  it('POSTs to /search with a Bearer key and the query', async () => {
    ...
    await makeTavilySearch({ apiKey: 'tvly-secret', fetchImpl }).search({
      query: 'hello',
  ...
  it('maps 429→rate-limit, 432/433→quota, 401→error', async () => {
    expect(
      await makeTavilySearch({ apiKey: 'k', fetchImpl: fakeFetch({ status: 429 }) }).search({
        query: 'q',
      }),
    ).toEqual({ status: 'limit', kind: 'rate-limit' });
    expect(
      await makeTavilySearch({ apiKey: 'k', fetchImpl: fakeFetch({ status: 432 }) }).search({
        query: 'q',
      }),
    ).toEqual({ status: 'limit', kind: 'quota' });
    expect(
      await makeTavilySearch({ apiKey: 'k', fetchImpl: fakeFetch({ status: 401 }) }).search({
        query: 'q',
      }),
    ).toMatchObject({ status: 'error' });
  });
  ...
  it('SC-1: a throw becomes an error outcome', async () => {
    ...
    expect(await makeTavilySearch({ apiKey: 'k', fetchImpl }).search({ query: 'q' })).toMatchObject(
      { status: 'error' },
    );
  });
});
