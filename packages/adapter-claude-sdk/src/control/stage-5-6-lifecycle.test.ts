...
/**
 * Control-spike stages 5-6 — context over time (compaction, mid-session injection) and
 * state ownership (session persistence, resume, checkpointing).
 *
...
describe('stage 5 — context over time: is compaction reachable from outside?', () => {
  ...
  it('gives a PreCompact handler no channel to shape or veto the compaction', () => {
    // THE decisive structural finding for M4. Every hook event that can influence its subject
    // does so through a `<Event>HookSpecificOutput` member of `SyncHookJSONOutput`. There is
    // no `PreCompactHookSpecificOutput` and no `PostCompactHookSpecificOutput` in the shipped
    // typings at all, so the richest thing a PreCompact handler may return is the generic
    // envelope (`continue` / `decision` / `systemMessage` / `reason`).
    //
