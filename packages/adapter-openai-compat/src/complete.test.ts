...
// The shared code path, exercised through one spec; the per-provider deltas
// (endpoints, reasoning fields) have their own describes below.
describe('makeOpenAiCompatComplete (shared loop)', () => {
  ...
  it('maps tool + assistant messages onto the wire', async () => {
    const captured: Captured = {};
    const complete = makeOpenAiCompatComplete(deepseekSpec, {
      apiKey: 'sk-1',
      model: 'm',
  ...
  it('captures reasoning_content as the completion reasoning, undefined when absent', async () => {
    const withReasoning = makeOpenAiCompatComplete(deepseekSpec, {
      apiKey: 'sk-1',
      model: 'm',
      fetchImpl: fakeFetch(
        {
    ...
    const noReasoning = makeOpenAiCompatComplete(deepseekSpec, {
      apiKey: 'sk-1',
      model: 'm',
  ...
  it('throws with the spec id and status on a non-ok response', async () => {
    const complete = makeOpenAiCompatComplete(deepseekSpec, {
      apiKey: 'sk-1',
      model: 'm',
      fetchImpl: fakeFetch({}, {}, false, 429),
    });
    await expect(drain(complete([{ role: 'user', content: 'go' }], []))).rejects.toThrow(
      'deepseek chat/completions failed: 429',
    );
  });
  ...
  // The shared wire schema carries the `.nullish()` fix for every provider — the identical
  // null-vs-undefined trap is already documented for `usage` in wire.ts.
...
describe('deepseekSpec request deltas', () => {
  it('sends reasoning_effort for an effort selection (coa max → the top rung)', async () => {
    const captured: Captured = {};
    const complete = makeOpenAiCompatComplete(deepseekSpec, {
      apiKey: 'sk-1',
      model: 'm',
  ...
  it('collapses the graded selections onto the two real rungs', () => {
    expect(deepseekSpec.reasoningBody({ mode: 'effort', effort: 'low' })).toEqual({
      reasoning_effort: 'high',
    });
    expect(deepseekSpec.reasoningBody({ mode: 'effort', effort: 'high' })).toEqual({
      reasoning_effort: 'high',
    });
    expect(deepseekSpec.reasoningBody({ mode: 'effort', effort: 'xhigh' })).toEqual({
      reasoning_effort: 'max',
    });
    // A budget selection has no DeepSeek surface; nothing is sent (the model default).
    expect(deepseekSpec.reasoningBody({ mode: 'budget', budgetTokens: 1_000 })).toEqual({});
    expect(deepseekSpec.reasoningBody(undefined)).toEqual({});
  });

  it('disables thinking (non-thinking mode) for the off selection', async () => {
    const captured: Captured = {};
    const complete = makeOpenAiCompatComplete(deepseekSpec, {
      apiKey: 'sk-1',
      model: 'm',
...
describe('longcatSpec request deltas', () => {
  it('posts to the LongCat chat endpoint', async () => {
    const captured: Captured = {};
    const complete = makeOpenAiCompatComplete(longcatSpec, {
      apiKey: 'sk-1',
      model: 'LongCat-2.0',
  ...
  it('enables thinking for an effort selection, and disables it for off', async () => {
    const enabled: Captured = {};
    await drain(
      makeOpenAiCompatComplete(longcatSpec, {
        apiKey: 'sk-1',
        model: 'm',
        reasoning: { mode: 'effort', effort: 'max' },
        fetchImpl: fakeFetch(textResponse, enabled),
      })([{ role: 'user', content: 'go' }], []),
    );
    expect(enabled.body?.['thinking']).toEqual({ type: 'enabled' });
    // LongCat has no graded reasoning_effort, so the effort level is not forwarded.
    expect(enabled.body?.['reasoning_effort']).toBeUndefined();
    ...
    await drain(
      makeOpenAiCompatComplete(longcatSpec, {
        ...
        model: 'm',
        reasoning: { mode: 'off' },
        fetchImpl: fakeFetch(textResponse, disabled),
      })([{ role: 'user', content: 'go' }], []),
    );
