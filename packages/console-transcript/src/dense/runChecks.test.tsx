...
describe('RunChecks', () => {
  it('renders a mark + name per check and a passed/failed footer', () => {
    ...
    expect(container.querySelector('[aria-label="passed" i]')).not.toBeNull();
    expect(container.querySelector('[aria-label="failed" i]')).not.toBeNull();
  ...
  it('renders only the passed count when nothing failed', () => {
    ...
    expect(container.querySelector('[aria-label="failed" i]')).toBeNull();
  });
