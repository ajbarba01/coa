...
/**
 * Map coa's faithful reasoning selection to DeepSeek's real surface: `off` → thinking
 * disabled (non-thinking); an effort → `high`, except `xhigh`/`max` → `max` (DeepSeek's
 * only two effort rungs); `budget`/absent → the V4 default (nothing sent).
 */
function toDeepSeekReasoning(
  reasoning: ModelSelection['reasoning'],
): DeepSeekReasoning | undefined {
  if (reasoning === undefined) return undefined;
  if (reasoning.mode === 'off') return { kind: 'disabled' };
  if (reasoning.mode === 'effort') {
    return {
      kind: 'effort',
      effort: reasoning.effort === 'xhigh' || reasoning.effort === 'max' ? 'max' : 'high',
    };
  }
  return undefined;
}
...
export class DeepSeekAdapter implements RuntimeAdapter {
  readonly #init: DeepSeekAdapterInit;
  ...
  async runLoop(_sessionConfig: SessionConfig): Promise<void> {
    ...
    if (apiKey === undefined) {
      throw new Error('deepseek: no API key — set DEEPSEEK_API_KEY or add an env-var account');
    }
    const reasoning = toDeepSeekReasoning(this.#init.model?.reasoning);
